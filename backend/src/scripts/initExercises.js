...
async function initExercises() {
  try {
    ...
    const exercises = [
      ...
      ['exercise1','排序查询（ORDER BY）','【场景】西区大棚数据追溯\n\n查询西区大棚的所有监测记录，按监测时间降序排列（最新数据优先）。','intermediate','排序查询','使用 WHERE 过滤 location，再用 ORDER BY monitor_time DESC 降序排列。',"SELECT * FROM sensor_monitor WHERE location = '西区大棚' ORDER BY monitor_time DESC",'返回西区大棚记录（按时间降序）','查询结果排序',7],
      ['exercise1','聚合函数查询（AVG / MAX / COUNT）','【场景】东区农田环境趋势分析\n\n统计东区农田温度监测的平均值、最高值和记录总数。','intermediate','聚合函数',"使用 AVG(monitor_value)、MAX(monitor_value)、COUNT(*) 三个聚合函数，WHERE 过滤 location 和 monitor_type。","SELECT AVG(monitor_value) AS avg_temp, MAX(monitor_value) AS max_temp, COUNT(*) AS record_count FROM sensor_monitor WHERE location = '东区农田' AND monitor_type = '温度'",'返回平均温度、最高温度、记录总数','使用集函数',8],
      ['exercise1','多重条件组合查询','【场景】跨区域异常排查\n\n查询 2025 年 11 月 2 日期间，南区果园或北区菜地的正常状态监测记录。','intermediate','多重条件组合',"使用 WHERE 组合 monitor_time 日期范围、(location = '南区果园' OR location = '北区菜地') 和 status = '正常'，注意 OR 条件需用括号。","SELECT * FROM sensor_monitor WHERE monitor_time >= '2025-11-02 00:00:00' AND monitor_time < '2025-11-03 00:00:00' AND (location = '南区果园' OR location = '北区菜地') AND status = '正常'",'返回指定日期指定区域的正常记录','选择元组-多重条件',9],
